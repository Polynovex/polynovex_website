...
//   const DropdownMenu = ({ anchorEl, open, onClose, items }) => (
//   <Menu
//     anchorEl={anchorEl}
//     open={open}
//     onClose={onClose}
...
//         <Link to={path} className="text-black">{label}</Link>
//       </MenuItem>
//     ))}
//   </Menu>
// );
...
//             >
//               {isMenuOpen ? <X className="h-6 w-6" /> : <AiOutlineMenu  className="h-6 w-6" />}
//             </button>
//           </div>
//         </div>
...
const Header = () => {
  ...
  return (
    <header className="bg-brand-white shadow-lg fixed w-[100%] top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <div className="flex-shrink-0">
            <Link to="/" className="text-2xl font-bold font-myriad text-brand-black">
              <img src="/images/logo.png" alt="Polynovex Logo" style={{ width: "100px", height: "50px" }} />
            </Link>
          </div>
          ...
          <nav className="hidden md:flex space-x-8 items-center">
            <Link
              to="/"
              className={`px-3 py-2 rounded-md text-sm font-medium font-arial transition-colors ${
                isActive('/') ? 'text-brand-black bg-gray-50' : 'text-gray-700 hover:text-brand-black hover:bg-gray-50'
              }`}
            >
              Home
            </Link>
            <Link
              to="/about"
              className={`px-3 py-2 rounded-md text-sm font-medium font-arial transition-colors ${
                isActive('/about') ? 'text-brand-black bg-gray-50' : 'text-gray-700 hover:text-brand-black hover:bg-gray-50'
              }`}
            >
              About
            </Link>

            {/* Services Dropdown using MUI */}
            <div>
              <Button
                onClick={handleOpen}
                endIcon={<ChevronDown className="h-4 w-4" />}
                // className="normal-case text-gray-700 hover:text-brand-black hover:bg-gray-50 px-3 py-2 font-arial text-sm"
               className="flex text-brand-black items-center !capitalize px-3 py-2 rounded-md text-sm font-medium font-arial !text-gray-700 hover:text-brand-black hover:bg-gray-50 transition-colors"
              >
                Services
              </Button>
              <Menu
              ...
              >
                {services.map((service) => (
                  <MenuItem key={service.path} onClick={handleClose}>
                    <Link to={service.path} className="text-gray-800 w-full">
                      {service.name}
                    </Link>
                  </MenuItem>
                ))}
            ...
            <Link
              to="/contact"
              className={`px-3 py-2 rounded-md text-sm font-medium font-arial transition-colors ${
                isActive('/contact') ? 'text-brand-black bg-gray-50' : 'text-gray-700 hover:text-brand-black hover:bg-gray-50'
              }`}
            >
              Contact
            </Link>
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-gray-50">
              <Link
                to="/"
                className="block px-3 py-2 rounded-md text-base font-medium font-arial text-gray-700 hover:text-brand-black hover:bg-gray-100"
                onClick={() => setIsMenuOpen(false)}
              >
                Home
              </Link>
              ...
              {services.map((service) => (
                <Link
                  key={service.path}
                  to={service.path}
                  className="block pl-6 pr-3 py-2 rounded-md text-sm font-arial text-gray-700 hover:text-brand-black hover:bg-gray-100"
                  onClick={() => setIsMenuOpen(false)}
                >
                  {service.name}
                </Link>
